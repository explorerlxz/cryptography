...
void add_round_key(uint8_t *state, uint8_t *w, uint8_t r) {
    ...
    for (c = 0; c < Nb; c++) {
        ...
        printf("       %.2x %.2x %.2x %.2x \n", w[4*Nb*r+4*c+0],w[4*Nb*r+4*c+1], w[4*Nb*r+4*c+2], w[4*Nb*r+4*c+3] );
	}
    int i;
}
...
void key_expansion(uint8_t *key, uint8_t *w) {
	...
	for (i = Nk; i < len; i++) {
		...
		if (i%Nk == 0) {
			...
			rot_word(tmp);//move left
			sub_word(tmp);//look table
		...
		w[4*i+0] = w[4*(i-Nk)+0]^tmp[0];//"^" means bitwise XOR
...
int main(int argc, char *argv[]) {
	...
	*/
	...
	//for (i = 0; i < 4; i++) {
	//	printf("%.2x %.2x %.2x %.2x ", out[4*i+0], out[4*i+1], out[4*i+2], out[4*i+3]);
	//}
	...
	//printf("msg:\n");
	//for (i = 0; i < 4; i++) {
	//	printf("%.2x %.2x %.2x %.2x ", in[4*i+0], in[4*i+1], in[4*i+2], in[4*i+3]);
	//}
